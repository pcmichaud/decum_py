...
df = pd.read_csv('output/values_bo.csv')
...
df.to_csv('output/values_bo_with_deltas.csv')
...
plt.ylabel('cdf')
plt.xlabel('$\\delta$')
plt.savefig('output/deltas_bo.eps',format='eps')
plt.show()
...
print(table)
table.round(3).to_latex('output/deltas_by_product_bo.tex')
