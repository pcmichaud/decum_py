...
# estimates
ssd =  7980.57
sigmas = np.load('output/sigmas_nokappa.npy')
pars =  np.load('output/estimates_nokappa.npy')

# standard errors
es = pd.read_csv('output/within_residuals_nokappa.csv',dtype=np.float64)
es['respid'] = es['respid'].astype('int64')
es.set_index('respid',inplace=True)
gs = pd.read_csv('output/gradients_nokappa.csv',dtype=np.float64)
...
table.round(3).to_latex('output/table_estimates_nokappa.tex',escape=False)
