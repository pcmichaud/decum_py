...
df = pd.read_csv('output/values_ez.csv')
...
df.to_csv('output/values_ez_with_deltas.csv')
...
plt.ylabel('cdf')
plt.xlabel('$\\delta$')
plt.savefig('output/deltas_ez.eps',format='eps')
plt.show()
...
print(table)
table.round(3).to_latex('output/deltas_by_product_ez.tex')
